...
@interface EspTouchDelegateImpl2 : NSObject<ESPTouchDelegate>

@end

@implementation EspTouchDelegateImpl2
...
@property (nonatomic, strong) EspTouchDelegateImpl2 *_esptouchDelegate;
@end
...
{
    // do confirm
    __weak SmartConfigViewController *wSelf = self;
...
//                        [[[UIAlertView alloc]initWithTitle:@"Execute Result" message:mutableStr delegate:nil cancelButtonTitle:@"I know" otherButtonTitles:nil]show];
                        [wSelf showAlertMessage:@"Kết nối thành công" title:nil action:@"Đồng ý" completion:^{
                          
                            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                                if(wSelf.handleAddControl){
                                    wSelf.handleAddControl(self.qrCodeString);
                                }
                            });
...
//                        [[[UIAlertView alloc]initWithTitle:@"Execute Result" message:@"Esptouch fail" delegate:nil cancelButtonTitle:@"I know" otherButtonTitles:nil]show];
                        [self showAlertMessage:@"Kết nối thất bại" title:nil action:@"Đồng ý" completion:nil];
...
//                    [[[UIAlertView alloc] initWithTitle:@"Execute Result" message:[esptouchResult description] delegate:nil cancelButtonTitle:@"I know" otherButtonTitles: nil] show];
                    [self showAlertMessage:@"Kết nối thất bại" title:nil action:@"Đồng ý" completion:^{
...
-(void)showAlertMessage:(NSString *)message title:(NSString *)title action:(NSString *)action completion:(void(^)(void))completion{
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:title message:message preferredStyle:UIAlertControllerStyleAlert];
    
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:action style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        if(completion){
            completion();
        }
    }];
...
- (void)viewDidLoad
{
    ...
    self._esptouchDelegate = [[EspTouchDelegateImpl2 alloc]init];
    [self enableConfirmBtn];
    
    AppDelegate *delegate = (AppDelegate *)[UIApplication sharedApplication].delegate;
    NSDictionary *netInfo = [delegate fetchNetInfo];
    self.ssidLabel.text = [netInfo objectForKey:@"SSID"];
    self.bssid = [netInfo objectForKey:@"BSSID"];
    self._confirmCancelBtn.layer.cornerRadius = 3.0;
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    self.memberView.hidden = ![[FirebaseHelper sharedInstance] isAdmin];
...
{
    self.confirmBottomConstrain.constant = 100;
...
{
    self.confirmBottomConstrain.constant = 40;
    [UIView animateWithDuration:0.25 animations:^{
        [self.view updateConstraintsIfNeeded];
    }];
}
